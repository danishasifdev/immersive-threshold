...
export default function SectionInhabit({
  orbitEnabled,
  onToggleOrbit,
  onDissolve,
}: SectionInhabitProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(".inhabit-heading", {
        y: "108%",
        duration: 1.3,
        stagger: 0.12,
        ease: "power3.out",
      });
      gsap.from(".inhabit-body", {
        opacity: 0,
        y: 14,
        duration: 1,
        delay: 0.4,
          ...
          <div className="md:col-span-6">
            <div className="overflow-hidden mb-0.5">
              <div className="inhabit-heading">
                <h2 className="t-heading text-threshold-parchment">
                  The system
                </h2>
              </div>
            </div>
            ...
            <p className="inhabit-body t-body text-threshold-parchment/60 max-w-[38ch] mb-12">
              ...
              Then — when it has been fully examined — release it. Let it become
              what it needs to become next.
            </p>
            ...
            <p className="inhabit-body t-mono text-[0.55rem] text-threshold-parchment/35 mt-3">
              {orbitEnabled
                ? "↑ drag the 3D scene above to explore"
                : "↑ free exploration enables drag-to-rotate"}
            </p>
          </div>
              ...
              {PRODUCT_PRINCIPLES.map(([key, val], i) => (
                ...
                >
                  <div className="flex justify-between items-start gap-4">
                    <span className="t-mono text-[0.6rem] text-threshold-parchment/75 shrink-0">
                      {key}
                    </span>
                    <span className="t-mono text-[0.55rem] text-threshold-parchment/45 text-right">
                      {val}
                    </span>
                  </div>
                </div>
